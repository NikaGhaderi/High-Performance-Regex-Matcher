#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <pthread.h>
#include <hs/hs.h>
#include <time.h>
#include <sys/stat.h>
#include <unistd.h>
#include <errno.h>

// Structure to hold match information for a single line
typedef struct {
    int line_number;
    unsigned int *matches;
    size_t match_count;
} match_result_t;

// Structure for passing data to each thread
typedef struct {
    int thread_id;
    hs_database_t *database;
    char **lines;
    size_t start_line;
    size_t end_line;
    match_result_t *results;
    hs_scratch_t *scratch;
    unsigned long long total_match_count;
} thread_data_t;

// Forward declaration for the match event handler callback
static int on_match(unsigned int id, unsigned long long from,
                    unsigned long long to, unsigned int flags, void *context);

// Function to read a file into an array of strings (lines)
char **read_lines(const char *filename, size_t *line_count, size_t *total_bytes) {
    FILE *file = fopen(filename, "r");
    if (!file) {
        perror("Error opening file");
        return NULL;
    }

    char **lines = NULL;
    size_t capacity = 0;
    size_t count = 0;
    char *line_buf = NULL;
    size_t len = 0;
    ssize_t read;
    size_t bytes = 0;

    while ((read = getline(&line_buf, &len, file)) != -1) {
        if (count >= capacity) {
            capacity = (capacity == 0) ? 256 : capacity * 2;
            char **new_lines = realloc(lines, capacity * sizeof(char *));
            if (!new_lines) {
                perror("Memory allocation failed for lines array");
                goto cleanup_error;
            }
            lines = new_lines;
        }

        if (read > 0 && line_buf[read - 1] == '\n') {
            line_buf[read - 1] = '\0';
            read--;
        }

        lines[count] = strdup(line_buf);
        if (!lines[count]) {
            perror("Memory allocation failed for line");
            goto cleanup_error;
        }

        bytes += read;
        count++;
    }

    free(line_buf);
    fclose(file);
    *line_count = count;
    *total_bytes = bytes;
    return lines;

cleanup_error:
    free(line_buf);
    fclose(file);
    for (size_t i = 0; i < count; i++) free(lines[i]);
    free(lines);
    return NULL;
}

// Function to free the memory allocated for the lines array
void free_lines(char **lines, size_t count) {
    if (lines) {
        for (size_t i = 0; i < count; i++) {
            free(lines[i]);
        }
        free(lines);
    }
}

// Hyperscan callback function, triggered on each match
static int on_match(unsigned int id, unsigned long long from,
                    unsigned long long to, unsigned int flags, void *context) {
    (void)from; (void)to; (void)flags;
    
    match_result_t *result = (match_result_t *)context;
    
    size_t new_count = result->match_count + 1;
    unsigned int *new_matches = realloc(result->matches, new_count * sizeof(unsigned int));
    if (!new_matches) {
        fprintf(stderr, "Failed to reallocate memory for matches\n");
        return 1; // Signal error to Hyperscan, stop scanning
    }
    
    result->matches = new_matches;
    result->matches[result->match_count] = id;
    result->match_count = new_count;
    
    // With HS_FLAG_SINGLEMATCH, this callback is only called once per pattern,
    // so we can continue scanning for other patterns.
    return 0;
}

// The main function for each thread
void *process_lines(void *arg) {
    thread_data_t *data = (thread_data_t *)arg;
    
    for (size_t i = data->start_line; i < data->end_line; i++) {
        match_result_t *result = &data->results[i];
        result->line_number = i;
        result->matches = NULL;
        result->match_count = 0;
        
        hs_error_t err = hs_scan(data->database, data->lines[i], strlen(data->lines[i]), 
                                0, data->scratch, on_match, result);
        
        if (err != HS_SUCCESS) {
            fprintf(stderr, "Thread %d: Hyperscan scan error: %d on line %zu\n", data->thread_id, err, i);
        }
        
        data->total_match_count += result->match_count;
    }
    
    return NULL;
}

// Function to compile regex patterns into a Hyperscan database
hs_database_t *compile_patterns(char **patterns, size_t pattern_count, size_t *db_size) {
    hs_database_t *db = NULL;
    hs_compile_error_t *compile_err = NULL;
    
    const char **pattern_ptrs = (const char **)patterns;
    unsigned int *flags = malloc(pattern_count * sizeof(unsigned int));
    unsigned int *ids = malloc(pattern_count * sizeof(unsigned int));
    
    if (!flags || !ids) {
        perror("Memory allocation failed for flags/ids");
        free(flags); free(ids);
        return NULL;
    }
    
    // *** FIX: Set appropriate flags for all patterns ***
    // This is the critical fix. These flags enable common regex features.
    for (size_t i = 0; i < pattern_count; i++) {
        flags[i] = HS_FLAG_DOTALL | HS_FLAG_MULTILINE | HS_FLAG_SINGLEMATCH | HS_FLAG_UTF8;
        ids[i] = (unsigned int)i;
    }
    
    printf("Compiling %zu patterns with DOTALL, MULTILINE, and SINGLEMATCH flags...\n", pattern_count);
    hs_error_t err = hs_compile_multi(pattern_ptrs, flags, ids, (unsigned int)pattern_count,
                                     HS_MODE_BLOCK, NULL, &db, &compile_err);
    
    free(flags); free(ids);
    
    if (err != HS_SUCCESS) {
        fprintf(stderr, "Hyperscan compile error (code %d): %s\n", err, compile_err->message);
        if (compile_err->expression >= 0) {
            fprintf(stderr, "-> Error in pattern #%d: '%s'\n", compile_err->expression, 
                   patterns[compile_err->expression]);
        }
        hs_free_compile_error(compile_err);
        return NULL;
    }
    
    // *** FIX: Restore database size check. It will work now. ***
    if (db_size) {
        err = hs_database_size(db, db_size);
        if (err != HS_SUCCESS) {
            fprintf(stderr, "Error getting database size: %d\n", err);
            *db_size = 0;
        }
    }
    
    printf("Pattern compilation successful.\n");
    return db;
}

// Function to initialize one scratch space per thread
int init_scratch_spaces(hs_database_t *db, int num_threads, hs_scratch_t ***scratch_spaces_ptr) {
    if (!db) {
        fprintf(stderr, "Database is NULL, cannot allocate scratch spaces.\n");
        return 0;
    }
    
    hs_scratch_t **scratch_spaces = malloc(num_threads * sizeof(hs_scratch_t *));
    if (!scratch_spaces) {
        perror("Memory allocation failed for scratch spaces array");
        return 0;
    }
    
    for (int i = 0; i < num_threads; i++) {
        hs_error_t err = hs_alloc_scratch(db, &scratch_spaces[i]);
        if (err != HS_SUCCESS) {
            fprintf(stderr, "Error allocating scratch space for thread %d. Hyperscan error code: %d\n", i, err);
            if (err == HS_NOMEM) {
                fprintf(stderr, "-> Ran out of memory. The pattern set is likely too large and complex for the available system memory.\n");
            } else if (err == HS_INVALID) {
                fprintf(stderr, "-> Invalid parameter. The database may be invalid (this shouldn't happen after the compile fix).\n");
            }
            for (int j = 0; j < i; j++) hs_free_scratch(scratch_spaces[j]);
            free(scratch_spaces);
            return 0;
        }
    }
    
    *scratch_spaces_ptr = scratch_spaces;
    return 1;
}

// Function to write match results to the output file
int write_output(const char *filename, match_result_t *results, size_t count) {
    FILE *file = fopen(filename, "w");
    if (!file) {
        perror("Error opening output file");
        return 0;
    }
    for (size_t i = 0; i < count; i++) {
        if (results[i].match_count > 0) {
            for (size_t j = 0; j < results[i].match_count; j++) {
                fprintf(file, "%u%s", results[i].matches[j], (j < results[i].match_count - 1) ? "," : "");
            }
        }
        fprintf(file, "\n");
    }
    fclose(file);
    return 1;
}

// Function to append performance metrics to a CSV file
int write_metrics(const char *filename, int threads, double input_per_sec, 
                  double mbytes_per_sec, double match_per_sec, double latency) {
    FILE *file;
    struct stat st;
    int file_exists = (stat(filename, &st) == 0);
    
    file = fopen(filename, "a");
    if (!file) {
        perror("Error opening metrics file");
        return 0;
    }
    if (!file_exists) {
        fprintf(file, "threads,throughput_input_per_sec,throughput_mbytes_per_sec,throughput_match_per_sec,latency\n");
    }
    fprintf(file, "%d,%.2f,%.2f,%.2f,%.4f\n", 
            threads, input_per_sec, mbytes_per_sec, match_per_sec, latency);
    fclose(file);
    return 1;
}

int main(int argc, char *argv[]) {
    if (argc < 5) {
        fprintf(stderr, "Usage: %s <patterns_file> <input_file> <output_file> <metrics_file> [threads]\n", argv[0]);
        return 1;
    }
    
    const char *patterns_file = argv[1];
    const char *input_file = argv[2];
    const char *output_file = argv[3];
    const char *metrics_file = argv[4];
    int num_threads = (argc > 5) ? atoi(argv[5]) : 1;
    if (num_threads <= 0) num_threads = 1;
    
    printf("Starting with %d threads...\n", num_threads);
    
    size_t pattern_bytes;
    size_t pattern_count = 0;
    char **patterns = read_lines(patterns_file, &pattern_count, &pattern_bytes);
    if (!patterns) {
        fprintf(stderr, "Failed to read patterns file.\n");
        return 1;
    }
    printf("Loaded %zu patterns.\n", pattern_count);
    
    size_t db_size = 0;
    hs_database_t *database = compile_patterns(patterns, pattern_count, &db_size);
    if (!database) {
        free_lines(patterns, pattern_count);
        return 1;
    }
    printf("Database size: %.2f MB\n", db_size / (1024.0 * 1024.0));
    
    hs_scratch_t **scratch_spaces = NULL;
    if (!init_scratch_spaces(database, num_threads, &scratch_spaces)) {
        hs_free_database(database);
        free_lines(patterns, pattern_count);
        return 1;
    }
    printf("Successfully allocated scratch space for %d threads.\n", num_threads);
    
    size_t input_line_count = 0;
    size_t total_input_bytes = 0;
    char **input_lines = read_lines(input_file, &input_line_count, &total_input_bytes);
    if (!input_lines) {
        fprintf(stderr, "Failed to read input file.\n");
        goto cleanup_db;
    }
    printf("Loaded %zu input lines (%.2f MB).\n", input_line_count, total_input_bytes / (1024.0 * 1024.0));

    match_result_t *results = calloc(input_line_count, sizeof(match_result_t));
    if (!results) {
        perror("Failed to allocate memory for results");
        goto cleanup_input;
    }

    pthread_t *threads = malloc(num_threads * sizeof(pthread_t));
    thread_data_t *thread_data = malloc(num_threads * sizeof(thread_data_t));
    if (!threads || !thread_data) {
        perror("Failed to allocate memory for threads");
        goto cleanup_results;
    }

    struct timespec start_time, end_time;
    clock_gettime(CLOCK_MONOTONIC, &start_time);

    size_t lines_per_thread = input_line_count / num_threads;
    for (int i = 0; i < num_threads; i++) {
        thread_data[i].thread_id = i;
        thread_data[i].database = database;
        thread_data[i].lines = input_lines;
        thread_data[i].start_line = i * lines_per_thread;
        thread_data[i].end_line = (i == num_threads - 1) ? input_line_count : (i + 1) * lines_per_thread;
        thread_data[i].results = results;
        thread_data[i].scratch = scratch_spaces[i];
        thread_data[i].total_match_count = 0;
        pthread_create(&threads[i], NULL, process_lines, &thread_data[i]);
    }

    unsigned long long total_matches = 0;
    for (int i = 0; i < num_threads; i++) {
        pthread_join(threads[i], NULL);
        total_matches += thread_data[i].total_match_count;
    }

    clock_gettime(CLOCK_MONOTONIC, &end_time);
    
    double processing_time = (end_time.tv_sec - start_time.tv_sec) +
                           (end_time.tv_nsec - start_time.tv_nsec) / 1e9;
    
    double input_per_sec = (processing_time > 0) ? input_line_count / processing_time : 0;
    double mbytes_per_sec = (processing_time > 0) ? (total_input_bytes / (1024.0 * 1024.0)) / processing_time : 0;
    double match_per_sec = (processing_time > 0) ? total_matches / processing_time : 0;
    double latency = (input_line_count > 0) ? (processing_time * 1000.0) / input_line_count : 0;
    
    write_output(output_file, results, input_line_count);
    write_metrics(metrics_file, num_threads, input_per_sec, mbytes_per_sec, match_per_sec, latency);
    
    printf("Processing completed.\n");
    printf("  - Total matches found: %llu\n", total_matches);
    printf("  - Throughput: %.2f lines/sec\n", input_per_sec);
    
cleanup_results:
    free(threads);
    free(thread_data);
    for (size_t i = 0; i < input_line_count; i++) free(results[i].matches);
    free(results);
cleanup_input:
    free_lines(input_lines, input_line_count);
cleanup_db:
    hs_free_database(database);
    for (int i = 0; i < num_threads; i++) hs_free_scratch(scratch_spaces[i]);
    free(scratch_spaces);
    free_lines(patterns, pattern_count);
    
    return 0;
}
